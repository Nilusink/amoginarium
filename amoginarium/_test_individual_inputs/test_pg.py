...
user32.GetRawInputData.argtypes = [wintypes.LPARAM, wintypes.UINT, ctypes.c_void_p, ctypes.POINTER(wintypes.UINT),
                                   wintypes.UINT]
user32.GetRawInputData.restype = wintypes.UINT


class RAWINPUTDEVICE(ctypes.Structure):
    _fields_ = [("usUsagePage", wintypes.WORD), ("usUsage", wintypes.WORD),
...
class RAWMOUSE(ctypes.Structure):
    ...
    _anonymous_ = ("buttons",)
    _fields_ = [("usFlags", wintypes.USHORT), ("buttons", _BUTTONS),
...
class RAWINPUT(ctypes.Structure):
    class _DATA(ctypes.Union):
        _fields_ = [("mouse", RAWMOUSE), ("keyboard", RAWKEYBOARD), ("hid", wintypes.DWORD * 4)]
...
def register_raw_input(hwnd):
    ...
    rids[0].usUsagePage, rids[0].usUsage, rids[0].dwFlags, rids[0].hwndTarget = 0x01, 0x02, 0, hwnd  # Mouse
    rids[1].usUsagePage, rids[1].usUsage, rids[1].dwFlags, rids[1].hwndTarget = 0x01, 0x06, 0, hwnd  # Keyboard
...
def custom_wndproc(hwnd, msg, wparam, lparam):
    if msg == WM_INPUT:
        ...
        if size.value > 0:
            ...
            if user32.GetRawInputData(lparam, RID_INPUT, buffer, ctypes.byref(size), ctypes.sizeof(RAWINPUTHEADER)) > 0:
                ...
                if raw.header.dwType == RIM_TYPEMOUSE:
                    ...
                    if dx != 0 or dy != 0:
                        ...
                        mice_state[hDev]['x'] = max(0, min(WINDOW_WIDTH, mice_state[hDev]['x']))
                        mice_state[hDev]['y'] = max(0, min(WINDOW_HEIGHT, mice_state[hDev]['y']))
                ...
                # --- Process Keyboard Data ---
                    ...
                    if hDev not in keyboards_state:
                        keyboards_state[hDev] = {
                            ...
                            'y': WINDOW_HEIGHT // 2,
                            'color': (random.random(), random.random(), random.random()),
                            'keys': {VK_W: False, VK_A: False, VK_S: False, VK_D: False}
                        }
...
def apply_keyboard_movement():
    ...
    for state in keyboards_state.values():
        if state['keys'][VK_W]: state['y'] -= speed
        if state['keys'][VK_S]: state['y'] += speed
        if state['keys'][VK_A]: state['x'] -= speed
        if state['keys'][VK_D]: state['x'] += speed
