...
class GraphWidget(ctk.CTkFrame):
    def __init__(self, master, graph_id, title_text, app, **kwargs):
        ...
        self.btn_full = ctk.CTkButton(self.header, text="⛶", width=30, height=25,
                                      command=self.toggle_fullscreen, fg_color="transparent", hover_color=("gray75", "gray40"))
        self.btn_full.pack(side=tk.RIGHT, padx=2)

        self.btn_min = ctk.CTkButton(self.header, text="▼", width=30, height=25,
                                     command=self.toggle_minimize, fg_color="transparent", hover_color=("gray75", "gray40"))
        ...
        # Loading Overlay
        self.loading_label = ctk.CTkLabel(self, text="Processing Data...", font=("Arial", 16, "bold"), fg_color=app.graph_bg)
    ...
    def toggle_fullscreen(self):
        ...
        self.app.root.after(10, lambda: self.app.set_fullscreen_widget(self if self.is_fullscreen else None))

    def setup_interactive_legend(self):
...
class DebugAnalyzerApp:
    ...
    def apply_theme_settings(self):
        ...
        plt.rcParams.update({
            ...
            "text.color": self.mpl_fg, "xtick.color": self.mpl_fg, "ytick.color": self.mpl_fg,
        })

    def sync_theme_to_widgets(self):
        ...
        try:
            ...
            for child in gw.toolbar.winfo_children():
                try:
                    ...
                    if isinstance(child, tk.Label): child.config(foreground=self.mpl_fg)
                    elif isinstance(child, tk.Button):
                        child.config(activebackground=self.pw_bg, bd=0, foreground=self.mpl_fg)
                        if icon_idx < len(icons):
                            child.config(image="", text=icons[icon_idx], font=("Segoe UI Symbol", 14))
                            icon_idx += 1
    ...
    def _setup_ui(self):
        ...
        self.btn_expand = ctk.CTkButton(self.collapsed_sidebar, text="▶", width=30, height=30, command=self.expand_sidebar, fg_color="transparent")
        self.btn_expand.pack(pady=15, padx=5)

        self.paned_window = tk.PanedWindow(self.root, orient=tk.HORIZONTAL, bd=0, sashwidth=6, bg=self.pw_bg, opaqueresize=False)
        self.paned_window.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.paned_window.bind("<ButtonRelease-1>", lambda e: self.root.after(50, self.save_current_settings))
        ...
        ctk.CTkSwitch(sw_frame, text="Auto switch to new:", variable=self.auto_switch_var, command=self.save_current_settings).pack(side=tk.LEFT)

        self.auto_mode_seg = ctk.CTkSegmentedButton(inner_settings, values=["Oldest", "Newest", "Add"], command=self.on_strat_change)
        self.auto_mode_seg.set(self.settings.get("auto_switch_mode", "Oldest"))
        self.auto_mode_seg.pack(fill=tk.X, pady=(0, 5))
        ...
        ctk.CTkLabel(self.placeholder_frame, text="No runs selected or available.", font=("Arial", 20, "bold"), text_color=("gray60", "gray40")).place(relx=0.5, rely=0.5, anchor=tk.CENTER)
    ...
    def save_settings_no_focus(self):
    ...
    def save_current_settings(self, event=None):
        ...
        try: max_f = int(self.max_files_entry.get())
        except ValueError: max_f = -1

        self.root.focus_set()

        if 0 <= max_f < len(self.known_files) and max_f != self.settings.get("max_files", -1):
            num_to_delete = len(self.known_files) - max_f
            if not messagebox.askyesno("Confirm Deletion", f"Changing max files to {max_f} will delete the {num_to_delete} oldest run(s).\n\nDo you want to proceed?"):
    ...
    def on_strat_change(self, value):
    ...
    def set_fullscreen_widget(self, widget):
    ...
    def apply_layout(self):
        ...
        for g_r, g_l, col_idx in active_widgets:
            ...
            if g_r.is_minimized:
                ...
                g_r.content_frame.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
            ...
            if g_l.is_minimized:
                ...
                g_l.content_frame.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
            ...
            if self.settings["auto_switch_new"] and new_idents:
                ...
                for new_ident in new_idents:
                    if len(self.selected_identifiers) > 0:
                        filtered_sel = [i for i in self.selected_identifiers if i in mtime_map]
                        current_sel_sorted = sorted(filtered_sel, key=lambda x: mtime_map.get(x, 0))

                        mode = self.settings.get("auto_switch_mode", "Oldest")
                        if len(current_sel_sorted) > 0 and mode != "Add":
                            if mode == "Oldest": removed = current_sel_sorted[0]
                            else: removed = current_sel_sorted[-1]
    ...
    def get_data(self, identifier, graphic_filepath, logic_filepath, mtime):
    ...
    def _process_data_thread(self):
        try:
            ...
            self.root.after(0, lambda: self._apply_plots(processed_data, mode, is_multi, color_families))
        except Exception:
            self.root.after(0, self._cleanup_plotting_state)

    def _apply_plots(self, processed_data, mode, is_multi, color_families):
        for gw in self.graphs.values():
            gw.ax.clear()
        ...
        for pd in processed_data:
            ...
            target_r.ax.plot(pd["pygame_xs"], pd["pygame_ys"], label=f"{prefix}pygame", color=c_pygame)
            target_r.ax.plot(pd["logic_xs"], pd["logic_ys"], label=f"{prefix}logic", color=c_logic)
            target_r.ax_twin.plot(pd["bullets_xs"], pd["n_bullets"], color=c_ent, label=f"{prefix}entities", linestyle='--' if is_multi else '-')

            target_l.ax.scatter(pd["n_bullets"], pd["bullets_ys"], label=f"{prefix}loops", color=c_logic, alpha=0.5)
            target_l.ax.plot(pd["av_bullet_xs"], pd["av_bullet_ys"], label=f"{prefix}avg", color=c_ent, linewidth=2)
        ...
        for gw in self.graphs.values():
            ...
            if is_r_graph:
                ...
                if gw.ax_twin: gw.ax_twin.legend(lines1 + lines2, labels1 + labels2, loc='upper left', fontsize='small')
                else: gw.ax.legend(lines1 + lines2, labels1 + labels2, loc='upper left', fontsize='small')
    ...
    def _cleanup_plotting_state(self):
        for gw in self.graphs.values():
